defmodule Andy.PG2Communicator do
  ...
  def init([]) do
    # the pg2 group is the community's name
    group = Andy.community_name()
    {:ok, _pid} = :pg2.start()
    :ok = :pg2.create(group)
  ...
  def handle_cast(
        {:broadcast, info},
        %{group: group, community_name: community_name} = state
      ) do
    members = :pg2.get_members(group)

    Logger.info(
      "COMMUNICATOR #{inspect(Node.self())} broadcasting #{inspect(info)} to #{
        inspect(Node.list())
      }"
    )
  ...
  def handle_cast(
        {:communication, from_node, info, id_channel, community_name},
        %{id_channels: id_channels} = state
      ) do
    if from_node != Node.self() do
      Logger.info(
        "COMMUNICATOR #{inspect(Node.self())} heard #{inspect(info)} from #{inspect(from_node)} in community #{
          community_name
        } and with id channel #{id_channel}"
      )
      ...
      # not handled by anyone for now
      PubSub.notify_id_channel(id_channel, community_name)
      PubSub.notify_perceived(%{percept | ttl: @ttl, source: @name})
