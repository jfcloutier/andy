defmodule Andy.Conjectures do
  ...
  def competing_conjecture_names(conjecture) do
    competing_conjecture_names =
      Agent.get(
        ...
        fn state ->
          ...
          competing_descendants =
            Enum.map(competing_siblings, &descendant_names(&1, state.family_tree))
          ...
          non_competing_sibling_descendants =
            Enum.map(non_competing_siblings, &descendant_names(&1, state.family_tree))

          non_competing =
            [conjecture.name | descendants ++ non_competing_sibling_descendants]
            |> List.flatten()
    ...
    Logger.info(
      "Conjecture #{conjecture.name} has competitors #{inspect(competing_conjecture_names)}"
    )
  ...
  defp collect_family_tree(conjectures) do
    ...
    {family_tree, _} =
      Enum.reduce(
        roots,
        {%{}, nil},
        fn conjecture, acc -> analyse_parentage(conjecture, acc, conjectures) end
      )
          ...
          fn prediction, acc1 ->
            case prediction.believed do
              ...
              {_, believed_conjecture_name} ->
                prediction_predecessors =
                  predecessor_conjectures_of_prediction(prediction, conjecture)

                acc_predecessors =
                  (Map.get(acc1, believed_conjecture_name, []) ++ prediction_predecessors)
                  |> Enum.uniq()

                Logger.info(
                  "Adding predecessors #{inspect(prediction_predecessors)} to #{
