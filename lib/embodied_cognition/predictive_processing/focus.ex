defmodule Andy.Focus do
  ...
  defp deprioritize(competing_conjecture_name, conjecture, prediction_name, deprioritizations) do
    case find_deprioritization(deprioritizations, conjecture.name, competing_conjecture_name) do
      ...
      %Deprioritization{prediction_names: prediction_names} = deprioritization ->
        Logger.info(
          "Focus: Conjecture #{competing_conjecture_name} already deprioritized by #{
            conjecture.name
          }"
        )
      ...
      nil ->
        ...
        if reducing_priority == :none do
          ...
          Logger.info(
            "Focus: Deprioritizing conjecture #{competing_conjecture_name} by #{reducing_priority}"
          )
  ...
  defp find_deprioritization(deprioritizations, conjecture_name, competing_conjecture_name) do
    Enum.find(
      deprioritizations,
      &(&1.conjecture_name == conjecture_name and
          &1.competing_conjecture_name == competing_conjecture_name)
    )
  end
    ...
    Enum.reduce(
      ...
      fn %Deprioritization{
        ...
        if competing_conjecture_name == conjecture.name do
          ...
          to_priority =
            Andy.reduce_level_by(conjecture.priority, deprioritizing_conjecture.priority)
  ...
  defp focus_off(
         conjecture_name,
         prediction_name,
         %{deprioritizations: deprioritizations} = state
       ) do
    Logger.info(
      "Focus: Focus off conjecture #{conjecture_name} because belief prediction #{prediction_name} was validated: #{
        inspect(deprioritizations)
      }"
    )
    ...
    updated_deprioritizations =
      Enum.reduce(
        ...
        fn %Deprioritization{prediction_names: prediction_names} = deprioritization, acc ->
          if deprioritization.conjecture_name == conjecture_name and
               prediction_name in prediction_names do
    ...
    Logger.info(
      "Focused off conjecture #{conjecture_name} on prediction #{prediction_name}: #{
        inspect(updated_deprioritizations)
      }"
    )
  ...
  defp reprioritize(conjecture_name, deprioritizations) do
    ...
    Logger.info(
      "Focus: Reprioritizing conjecture #{conjecture_name} by reducing its base priority by #{
        effective_reduction
      }"
    )
  ...
  defp focus_off_unconditionally(conjecture_name, %{deprioritizations: deprioritizations} = state) do
    Logger.info(
      "Focus: Focus off conjecture #{conjecture_name} unconditionally because believer was terminated"
    )

    {updated_deprioritizations, to_reprioritize} =
      Enum.reduce(
        deprioritizations,
        {[], []},
        fn %{
    ...
    Logger.info(
      "Focused off conjecture #{conjecture_name} unconditionally: #{
        inspect(updated_deprioritizations)
      }"
    )
