defmodule Andy.Experience do
  ...
  def handle_event({:prediction_error, prediction_error}, state) do
    ...
    if fulfillment_index != nil do
      Logger.info(
        "Experience chose fulfillment #{fulfillment_index} to address #{inspect(prediction_error)}"
      )

      # Activate fulfillment
      PubSub.notify_fulfill(
        Fulfill.new(
          validator_name: prediction_error.validator_name,
  ...
  defp learn_from_success_or_failure(
    ...
    Logger.info(
      "Fulfillment data = #{inspect({fulfillment_index, fulfillment_count})} from validator #{
        validator_name
      }"
    )
    ...
    new_validator_stats =
      case Map.get(fulfillment_stats, validator_name) do
        nil ->
  ...
  defp choose_fulfillment_index(
    ...
    if fulfillment_count == 0 do
      ...
      ratings =
        for [successes, failures, _] <- Map.get(fulfillment_stats, validator_name, []) do
