defmodule Andy.Recall do
  ...
  # Calculate the probability that the values of a kind of percepts within a given time period add up to a target value
  def probability_of_perceived(
        {percept_about, {:sum, attribute, target_sum}, time_period} = _perceived
      ) do
    percepts = Memory.recall_percepts_since(percept_about, time_period)
  ...
  def probability_of_perceived({percept_about, predicate, time_period} = _perceived) do
    ...
    Logger.info(
      "Recalled #{percepts_count} percepts matching #{inspect(percept_about)} over #{
        inspect(time_period)
      }"
    )
  ...
  def probability_of_actuated({intent_about, {:sum, target_sum}, time_period} = _actuated) do
    ...
    Logger.info(
      "Sum of #{Enum.count(intents)} intents #{inspect(intent_about)} is #{actual_sum} out of target #{
        target_sum
      } in #{inspect(time_period)}"
    )
  ...
  # Calculate the probability that the values of a kind of actuated intents within a given time period add up to a target value
  def probability_of_actuated(
        {intent_about, {:sum, attribute, target_sum}, time_period} = _actuated
      ) do
    intents = Memory.recall_intents_since(intent_about, time_period)
    ...
    Logger.info(
      "Sum of #{attribute} of intents #{inspect(intent_about)} is #{actual_sum} out of target #{
        target_sum
      } in #{inspect(time_period)}"
    )
  ...
  def probability_of_actuated({intent_about, {:times, target_number}, time_period} = _actuated) do
    actual_number =
      Memory.recall_intents_since(intent_about, time_period)
      |> Enum.count()

    Logger.info(
      "#{actual_number} intents #{inspect(intent_about)} found out of target #{target_number} in #{
        inspect(time_period)
      }"
    )
